...
def Generate_TSNE_visual(dataloaders_dict,model,feature_layer,sub_dir,device,class_names):
      ...
      # Turn interactive plotting off, don't show plots
        ...
        #pdb.set_trace()
        if feature_layer == nn.Sequential():
            try:
                feature_layer = nn.Sequential(model.spectrogram_extractor,model.logmel_extractor)
            except:
                feature_layer = nn.Sequential(model.module.spectrogram_extractor,model.module.logmel_extractor)
      #TSNE visual of (all) data
        ...
        for phase in ['test']:    
            ...
            for idx, (inputs, classes,index)  in enumerate(dataloaders_dict[phase]):
                ...
                features = model(images) #Need to account for pretrained models
                ...
                #saved_imgs.append(images.view(images.size()[0],images.size()[1]).cpu().numpy())
                saved_imgs.append(images.cpu().permute(-2,1).numpy())
                #saved_imgs.append(images.view(64, 96000).cpu().numpy())
