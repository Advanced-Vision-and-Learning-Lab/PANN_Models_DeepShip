...
from Utils.PANN_models import ResNet38,MobileNetV1,Res1dNet31,Wavegram_Logmel_Cnn14,Cnn14_8k,Cnn14_16k
from Utils.customPANN import CustomPANN
...
def initialize_model(model_name, num_classes, in_channels, out_channels,
    ...
    # Baseline model
        ...
        #PANN models
            ...
            if use_pretrained: #Pretrained on AudioSet
                model_ft = Cnn14(sample_rate=32000, window_size=512*2, hop_size=160, mel_bins=64, fmin=50, 
                    fmax=8000, classes_num=527)
                model_ft.load_state_dict(torch.load('./PANN_Weights/Cnn14_mAP=0.431.pth')['model'])
            else:
                model_ft = Cnn14(sample_rate=32000, window_size=1024, hop_size=160, mel_bins=64, fmin=50, 
                    fmax=8000, classes_num=num_classes)
            ...
            input_size = 224 #Verify
             ...
             #feature parameters from repo
             if use_pretrained: #Pretrained on AudioSet
                 model_ft = Wavegram_Logmel_Cnn14(sample_rate=32000, window_size=1024, hop_size=320, mel_bins=64, fmin=50, 
                     fmax=8000, classes_num=527)
                 model_ft.load_state_dict(torch.load('./PANN_Weights/Wavegram_Logmel_Cnn14_mAP=0.439.pth')['model'])
             else:
                 model_ft = Wavegram_Logmel_Cnn14(sample_rate=32000, window_size=1024, hop_size=160, mel_bins=64, fmin=0, 
        ...
        elif model_name == 'Cnn14_8k':
             #feature parameters from repo
             if use_pretrained: #Pretrained on AudioSet
                 model_ft = Cnn14_8k(sample_rate=8000, window_size=256, hop_size=80, mel_bins=64, fmin=50, 
                     fmax=4000, classes_num=527)
                 model_ft.load_state_dict(torch.load('./PANN_Weights/Cnn14_8k_mAP=0.416.pth')['model'])
             else:
                 model_ft = Cnn14_8k(sample_rate=8000, window_size=1024, hop_size=160, mel_bins=64, fmin=0, 
                     fmax=None, classes_num=num_classes)
        ...
        elif model_name == 'Res1dNet31':
            #feature parameters from repo
            if use_pretrained: #Pretrained on AudioSet
                model_ft = Res1dNet31(sample_rate=32000, window_size=1024, hop_size=320, mel_bins=64, fmin=50, 
                    fmax=8000, classes_num=527)
                model_ft.load_state_dict(torch.load('./PANN_Weights/Res1dNet31_mAP=0.365.pth')['model'])
            else:
                model_ft = Res1dNet31(sample_rate=32000, window_size=1024, hop_size=160, mel_bins=64, fmin=0, 
                    fmax=None, classes_num=num_classes)
            ...
            #feature parameters from repo
            if use_pretrained: #Pretrained on AudioSet
                model_ft = MobileNetV1(sample_rate=32000, window_size=1024, hop_size=320, mel_bins=64, fmin=50, 
                    fmax=8000, classes_num=527)
                model_ft.load_state_dict(torch.load('./PANN_Weights/MobileNetV1_mAP=0.389.pth')['model'])
            else:
                model_ft = MobileNetV1(sample_rate=32000, window_size=1024, hop_size=160, mel_bins=64, fmin=0, 
                    fmax=None, classes_num=num_classes)
        ...
        #     input_size = 224 #Verify
            ...
            if use_pretrained: #Pretrained on AudioSet
                ...
                model_ft = Cnn14(sample_rate=16000, window_size=512, hop_size=160, mel_bins=64, fmin=0, 
                    fmax=None, classes_num=num_classes)
            ...
            #feature parameters from repo
            if use_pretrained: #Pretrained on AudioSet
                model_ft = ResNet38(sample_rate=32000, window_size=1024, hop_size=320, mel_bins=64, fmin=50, 
                    fmax=8000, classes_num=527)
                model_ft.load_state_dict(torch.load('./PANN_Weights/ResNet38_mAP=0.434.pth')['model'])
            else:
                model_ft = ResNet38(sample_rate=32000, window_size=1024, hop_size=160, mel_bins=64, fmin=0, 
                    fmax=None, classes_num=num_classes)
    ...
    if model_name == 'CNN_14' or 'Cnn14_8k' or 'Cnn14_16k' or 'ResNet38' or 'MobileNetV1' or 'Res1dNet31' or 'Wavegram_Logmel_Cnn14':
        feature_layer = nn.Sequential()
        model_ft_custom = CustomPANN(model_ft)
    else:
    ...
    return model_ft_custom, input_size, feature_layer
