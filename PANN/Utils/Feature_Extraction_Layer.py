...
class Feature_Extraction_Layer(nn.Module):
    def __init__(self, input_feature, sample_rate=16000, window_length=250, 
                 hop_length=64, RGB=False, eps=10e-6, log_normal = False):
        ...
        #Set normalization
        if log_normal:
            self.normalization = self.log_normal
        else:
    ...
    def log_normal(self, x): #Make separate layer (apply transform before zero padding)
        
        #Apply log normalization
        #If eps is 1, zeros will be zero after transform
